...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    return GoodsStockNameCellHeight;
}

-(WXUITableViewCell*)goodsStockCell:(NSInteger)row{
    static NSString *identfier = @"stockCell";
    GoodsStockNameCell *cell = [_tableView dequeueReusableCellWithIdentifier:identfier];
    if(!cell){
        cell = [[GoodsStockNameCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identfier];
    }
...
-(void)loadGoodsStockInfo:(NSArray *)stockArr{
    ...
    if(_goodsViewType == GoodsStockView_Type_ShoppingCart){
        [buyBtn setTitle:@"加入购物车" forState:UIControlStateNormal];
    }
    
    __block GoodsStockView *blockSelf = self;
